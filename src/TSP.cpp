...
cost_t CostMatrix::reduce_rows() {
    cost_t suma = 0;
    std::vector<cost_t> vector = get_min_values_in_rows();
    for(int i = 0; i < matrix_.size(); i++){
        for(int j =0; j < matrix_.size(); j ++){
            if (matrix_[i][j] != INF) matrix_[i][j] -= vector[i];
        }
        suma += vector[i];
    }
    return suma;
...
std::vector<cost_t> CostMatrix::get_min_values_in_cols() const {
    std::vector<cost_t> min_values;
    for(int i = 0; i < matrix_.size(); i++){// col
        cost_t minimum = INF;
        for(int j = 0; j < matrix_.size(); j++)// row
            if(matrix_[j][i] < minimum) minimum = matrix_[j][i];
        if(minimum == INF) minimum = 0;
        min_values.push_back(minimum);
    }
    return min_values;
...
cost_t CostMatrix::reduce_cols() {
    cost_t suma = 0;
    std::vector<cost_t> vector = get_min_values_in_cols();
    for(int i = 0; i < matrix_.size(); i++){//rows
        for(int j = 0; j < matrix_.size(); j ++){//cols
            if(matrix_[i][j] != INF) matrix_[i][j] -= vector[j];
        }
        suma += vector[i];
    }
    return suma;
...
cost_t CostMatrix::get_vertex_cost(std::size_t row, std::size_t col) const {
    ...
    for(int i = 0; i < matrix_.size(); i++){
        if(i != col){
            if(matrix_[row][i] != INF && matrix_[row][i] <  min_col)
                min_col = matrix_[row][i];
        }
        if(i != row){
            if(matrix_[i][col] != INF && matrix_[i][col] < min_row)
